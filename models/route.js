...
const routemodel = mongoose.Schema({
  ...
  routeImages: {
    ...
    validate: {
      validator: (v) => Array.isArray(v) && v.length > 0,
      message:
        '-В маршруте должна быть хотя бы одно изображение и выбран аватар',
    },
  },
  avatar: {
    type: Object,
...
routemodel.methods.mkRouteDirectory = function () {
  ...
  if (!fs.existsSync(routeImageDirPath)) {
    fs.mkdirSync(routeImageDirPath);
  }
  const pointImagesDirPath = `./public/routes/${dirName}/pointimages`;
  if (!fs.existsSync(pointImagesDirPath)) {
    fs.mkdirSync(pointImagesDirPath);
  }
  if (
    fs.existsSync(dirPath) &&
    fs.existsSync(routeDirPath) &&
    fs.existsSync(routeImageDirPath) &&
    fs.existsSync(pointImagesDirPath)
  ) {
    return true;
  }
};
...
routemodel.methods.deleteFile = function (file) {
  ...
  this.routeImages = this.routeImages.filter(
