...
router.post('/forgotpass', async (req, res) => {
  ...
  if (!user) {res.status(401).json({message: `Такого Email ${req.body.email}, нет!!`})}
    ...
    .then(user => {
      transporter.sendMail(mailData, function(err) {
        if (err) {
          ...
          res.status(200).json({message: `Вам на почту ${user.email} отравлено письмо для смены пароля!`})
        }
      })
    })
    .catch(err => {
...
router.get('/resetpassword', async (req, res) => {
  ...
  const user = await UserModel.findOne({resetPasswordToken: req.query.key, resetPasswordExp: {$gt: Date.now()}})
  if (!user) {
    console.log('User not Found or Expired')
   return res.status(401).json({message: 'Пользователь не найден или итек срок'})
  }
...
router.post('/changepassword', auth, async (req, res) => {
  ...
  let user = null
  try {
    user = await UserModel.FindUserForAuth({email: req.body.email, password: req.body.oldPass})
    console.log(user)
