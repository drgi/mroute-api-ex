...
router.use(function timeLog(req, res, next) {
  ...
  router.post('/adddraft', auth, async (req, res) => {
    ...
    if (req.body.author.id != req.user._id) {return res.status(400).json({message: 'Author error'})}
    try {
    const route = await RouteModel({...req.body, isDraft: true})
    route.mkRouteDirectory()
    await route.save({validateBeforeSave: false})
    if (route) {
    res.status(200).json({routeDraft: route, message: 'Создан черновик маршрута'})
    }
    } catch (err) {
        console.log(err)
...
router.get('/:id', async (req, res)=>{
 ...
 router.delete('/:routeId', auth, async (req, res) => {
    ...
    try {
        const routeForDelete = await RouteModel.findOneAndDelete({_id: req.params.routeId})
 ...
 router.put('/add/:routeId', auth,  async (req, res) => {
    ...
    if (req.params.routeId === 'null'){
        return res.status(200).json({error: 'Ошибка, сначала создайте черновик маршрута'})      
    }
    try { 
    ...
    if (route){
      ...
      res.status(200).json({message: 'Неизвестная ошибка:('})
        ...
        res.status(200).json({message: `<ul><h6>Публикация не возможна!</h6>${e.join('')}</ul>`})
    }       
 })
 ...
 router.post('/images', auth, editRouteUpload, (req, res) => {        
     ...
     RouteModel.findById(req.body.routeId, function(err, route) {
         if (err) {return res.status(400).json({message: 'Маршрут не найден, создайте черновик'})}
         ...
         route.save({validateBeforeSave: false}, function(err, route) {
             ...
             if (route){
                 res.status(200).json({routeImages: route.routeImages, message: 'Файлы добавлены'})
             }
         })
     })
 ...
 router.delete('/images/del',(req, res) => {
    //1. Найти по ИД в базе
    RouteModel.findById(req.body.routeId, function(err, route) {
        if (err) {return res.status(400).json({message: 'Маршрут не найден'})}
        if (route.deleteFile(req.body.file)) {
            route.save({validateBeforeSave: false}, function(err, route) {
                ...
                if (route){
                    res.status(200).json({routeImages: route.routeImages, message: 'Файл удален'})
                }  
            })
        }
    })
...
router.post('/pointimages', auth, editRouteUpload, (req, res) => {        
    ...
    RouteModel.findById(req.body.routeId, function(err, route) {
        ...
        route.save({validateBeforeSave: false}, function(err, route) {
            ...
            if (route){
                res.status(200).json({pointImages: files, message: 'Файлы добавлены'})
            }
        })
    })
    //2. Записать файлы вернуть массив с путями
...
router.delete('/pointimages/del', async (req, res) => {
   // console.log(req.body)
    ...
    let route = await RouteModel.findById(req.body.routeId) 
             route.points = route.points.map(point => {
                if(point.id == req.body.pointId){
                    images = point.images = point.images.filter(image => image.path != req.body.file.path)
                    return point
                }
                return point
            })
