...
describe('Test Server Route auth/register POST', () => {
  ...
  test('Try register with with already registred User', async () => {
    ...
    expect(res.body).toHaveProperty('error');
    expect(res.body.error).toBe(
      `Пользователь с {"email":"${alrdRegUser.email}"}, уже зарегестрирован.`
    );
  });
});
...
describe('Test auth/changePassword, POST', () => {
  test('Try change Password with invalid token', async () => {
    const accessToken = `Bearer ${'invalid-jwt'}`;
    const res = await server
  ...
  test('Try change Password with valid token and credential', async () => {
    const jwt = generateJWTToken(ID, '50000ms');
    const accessToken = `Bearer ${jwt}`;

    const res = await server
      .post('/auth/changepassword')
      .set('Authorization', accessToken)
      .send({ email: testUser.email, oldPass: '123456', newPass: '12345678' });
    expect(res.statusCode).toBe(200);
  });
  test('Try change Password with valid token and invalid old password', async () => {
    const jwt = generateJWTToken(ID, '50000ms');
    const accessToken = `Bearer ${jwt}`;
    const res = await server
      .post('/auth/changepassword')
      .set('Authorization', accessToken)
      .send({ email: testUser.email, oldPass: 'invalid', newPass: '12345678' });
    expect(res.statusCode).toBe(400);
    expect(res.body).toHaveProperty('error');
  });
  test('Try change Password with valid token with invalid ID and valid credential', async () => {
    const jwt = generateJWTToken('invaid_id', '50000ms');
    const accessToken = `Bearer ${jwt}`;
    const res = await server
      .post('/auth/changepassword')
      .set('Authorization', accessToken)
  ...
  test('Try change Password with valid token and not registred email', async () => {
    const jwt = generateJWTToken(ID, '50000ms');
    const accessToken = `Bearer ${jwt}`;
    const res = await server
      .post('/auth/changepassword')
  ...
  test('Try change Password with valid token and wrong fields', async () => {
    const jwt = generateJWTToken(ID, '50000ms');
    const accessToken = `Bearer ${jwt}`;
    const res = await server
      .post('/auth/changepassword')
